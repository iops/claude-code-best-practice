# CLAUDE.md — Product-Led Collaboration (Extended)

> Based on [iops/product-mode](https://github.com/iops/product-mode/blob/main/CLAUDE.md) by Karpathy-derived team guidelines, with three additions from a personal `~/.claude/CLAUDE.md` to restore concrete in-the-moment heuristics.
>
> **Additions over upstream:**
> - **P3** gains a "Self-check rules" sub-section (200→50 rewrite trigger + senior-engineer overcomplicated test)
> - **P5** gains a Rules bullet on autonomous-loop framing (Karpathy's "give it success criteria and watch it go")
>
> Drop this in as your project or global CLAUDE.md, or use as a reference when authoring your own.
...
Self-check rules:
- **Length trigger:** If you wrote 200 lines and it could be 50, rewrite it. Long solutions are usually wrong solutions.
- **Senior engineer test:** Ask yourself - *"Would a senior engineer say this is overcomplicated?"* If yes, simplify. The test catches over-abstraction the scope rules miss.
...
- **Strong success criteria let the model loop independently.** Weak criteria ("make it work") trigger constant clarification requests mid-task; strong criteria ("invalid inputs rejected; user flow completes; tests cover each case") let the model self-verify and continue without pinging you. This is Karpathy's "don't tell it what to do, give it success criteria and watch it go" applied operationally.
...
Upstream (product-mode): MIT — see [github.com/iops/product-mode](https://github.com/iops/product-mode).
Personal additions (P3 self-check rules, P5 autonomous-loop bullet): same MIT terms.
Fork, adapt, and make it your team's own.
